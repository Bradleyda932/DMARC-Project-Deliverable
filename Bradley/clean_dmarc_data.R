...
# Ordered visit categories
ordered_levels <- c("1", "2", "3-5", "6-10", "11-25", "26-100", "101+")

# Prepare data
zip_50314_visits <- visitor_count_person %>%
  filter(zip_code == "50314") %>%
  count(visit_category) %>%
  mutate(
    visit_category = factor(visit_category, levels = ordered_levels),
    percent = n / sum(n) * 100,
    legend_label = paste0(visit_category, " (", round(percent, 1), "%)")
  )

# Preserve order of legend_label to match visit_category
zip_50314_visits$legend_label <- factor(zip_50314_visits$legend_label,
                                        levels = zip_50314_visits$legend_label[order(zip_50314_visits$visit_category)])

# Custom color palette (ordered)
final_colors <- c(
  "1" = "#D73027",      # red
  "2" = "#FC8D59",      # orange
  "3-5" = "#FEE08B",    # yellow
  "6-10" = "#91CF60",   # green
  "11-25" = "#4575B4",  # blue
  "26-100" = "#762A83", # purple
  "101+" = "#1dd3d3"    # cyan
)

# Name colors using ordered legend_label
legend_colors <- setNames(final_colors[zip_50314_visits$visit_category],
                          levels(zip_50314_visits$legend_label))

# Save PNG
png("50314_visit_frequency_piechart_sorted_legend_final.png", width = 1000, height = 600, res = 150)

ggplot(zip_50314_visits, aes(x = "", y = n, fill = legend_label)) +
  geom_bar(stat = "identity", width = 1, color = "white", size = 1) +
  coord_polar("y", start = 0) +
  scale_fill_manual(values = legend_colors, name = "Visit Category") +
  ...
  labs(title = "Visitor Frequency: Zip Code 50314")
